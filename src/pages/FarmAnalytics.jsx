...
export default function FarmAnalytics({
  ...
  const lastHealthFetchTime = useRef(0);
  ...
  const fetchFarmHealth = useCallback(
    async (silent = false) => {
      ...
      const now = Date.now();
      if (now - lastHealthFetchTime.current < DEBOUNCE_DELAY) {
        return;
      }
      lastHealthFetchTime.current = now;
  ...
  useEffect(() => {
    ...
    // Increased polling interval to 5 seconds for better performance
  ...
  // Memoize processed chart data with optimized lookups
  const chartData = useMemo(() => {
    ...
    if (!analyticsData?.error) {
      ...
      if (stats && Array.isArray(stats)) {
        // Create a map for O(1) lookups instead of O(n)
        const periodMap = new Map(
          defaultData.map((item, index) => [item.period, index])
        );
        ...
        stats.forEach((apiItem) => {
          ...
          let defaultIndex = periodMap.get(period);

          // Fallback to includes search if needed
          if (defaultIndex === undefined) {
            defaultIndex = defaultData.findIndex((item) => {
  ...
  const getDiseaseColor = useCallback(
    (disease, index) => {
      return (
        diseaseColors[disease] || `hsl(${(index * 137.5) % 360}, 70%, 50%)`
      );
    },
    [diseaseColors]
  );
  ...
  // Calculate summary stats from chartData based on date range
  const summaryStats = useMemo(() => {
    if (!chartData || chartData.length === 0) {
      return { totalScans: 0, healthScore: "0", diseaseScore: "0" };
    }

    // Sum up all predictions from the filtered chart data
    let totalScans = 0;
    let healthyCount = 0;

    chartData.forEach((item) => {
      totalScans += item.totalPredictions || 0;
      healthyCount += item.Healthy || 0;
    });

    const healthScore = totalScans > 0 ? (healthyCount / totalScans) * 100 : 0;
    const diseaseScore = totalScans > 0 ? (100 - healthScore).toFixed(1) : "0";
