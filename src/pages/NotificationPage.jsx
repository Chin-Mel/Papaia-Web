...
export default function NotificationPage() {
  ...
  // Generate hash from notifications data
  const generateHash = (data) => {
    const str = JSON.stringify(
      data.map((n) => ({ id: n.id, read: n.read, timestamp: n.timestamp }))
    );
    let hash = 0;
  ...
  // Silent polling for new notifications
  useEffect(() => {
    const checkForUpdates = async () => {
      if (!refreshNotifications) return;

      try {
        const newData = await refreshNotifications();
        if (newData) {
          const newHash = generateHash(newData);

          // Only update if hash changed
          if (lastHashRef.current !== null && lastHashRef.current !== newHash) {
            // Data changed - component will re-render automatically via context
    ...
    // Initial hash
    if (notifications.length > 0 && lastHashRef.current === null) {
      lastHashRef.current = generateHash(notifications);
    }
  ...
  const formatMessage = (disease) => {
    ...
    return `${article} <strong>${disease}</strong> disease has been detected on your farm!`;
  };
