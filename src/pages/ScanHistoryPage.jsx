...
const DiseaseCheckboxes = ({ selectedDiseases, onChange }) => {
  ...
  return (
    <div className="mt-2 space-y-2 p-3 bg-gray-50 rounded-lg border border-gray-200">
      <p className="text-xs font-medium text-gray-700 mb-2">Select Diseases:</p>
...
export default function ScanHistoryPage() {
  ...
  const filteredScans = useMemo(() => {
    ...
    // Farmer Name Search Filter
    if (filters.farmerSearch.trim()) {
      const searchTerm = filters.farmerSearch.toLowerCase().trim();
      filtered = filtered.filter((s) =>
  ...
  const handleFilterChange = useCallback((type, value) => {
    setFilters((prev) => {
      ...
      // Clear disease selection when status changes
