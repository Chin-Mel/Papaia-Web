...
const LoadingSpinner = () => (
  <div className="flex justify-center items-center py-8">
    <div className="relative w-10 h-10">
      ...
      <div className="absolute inset-0 border-3 border-emerald-600 rounded-full border-t-transparent animate-spin"></div>
    </div>
  </div>
);
...
export default function RecentActivities({ limit = 5 }) {
  ...
  const fetchActivities = useCallback(
    ...
    [limit, formatTime, getActivityStyle, getDefaultActivity]
  );
  ...
  useEffect(() => {
    if (activities.length > 0 && !isLoading) {
      ...
      if (container && container.offsetHeight > 0) {
        minHeightRef.current = container.offsetHeight;
      }
    }
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg shadow-slate-200/50 p-4 sm:p-6 w-full border border-slate-200/60">
      ...
      {isLoading ? (
        <div style={{ minHeight: minHeightRef.current || 300 }}>
          <LoadingSpinner />
        </div>
      ) : (
        <div
          ...
          className="space-y-3"
          style={{ minHeight: minHeightRef.current || "auto" }}
        >
          {activities.map((act) => {
