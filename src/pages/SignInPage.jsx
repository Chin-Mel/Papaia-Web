import { useState, useCallback, useEffect as useEffectSignIn } from "react";
import { Link, useNavigate as useNavigateSignIn } from "react-router-dom";
...
export default function SignInPage() {
  const navigate = useNavigateSignIn();
  ...
  useEffectSignIn(() => {
    if (usernameOrEmail) {
      debouncedValidation(usernameOrEmail);
    }
  }, [usernameOrEmail]);

  const handleReactivate = async () => {
    if (!deactivatedUserToken) return;
    setLoading(true);
  ...
  const handleSubmit = async (e) => {
    ...
    if (trimmedEmail.includes("@") && !validateEmail(trimmedEmail)) return;
    ...
    try {
      ...
      if (loginData.user?.status?.toLowerCase() === "deactivate") {
                ...
                <div className="space-y-4 sm:space-y-5">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      disabled={loading}
                      className="transition-all duration-150 active:scale-95 cursor-pointer w-full h-10 sm:h-11 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white text-sm sm:text-base font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="transition-all duration-150 active:scale-95 cursor-pointer w-full h-10 sm:h-11 border-2 border-gray-300 text-gray-700 text-sm sm:text-base font-medium rounded-lg hover:bg-gray-50 disabled:opacity-50"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-end">
                      <Link
                        to="/forgot-password"
                        className="text-xs sm:text-sm text-orange-500 hover:text-orange-600 hover:underline"
                      >
                        Forgot password?
                      </Link>
                    </div>
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full h-10 sm:h-11 bg-gradient-to-r bg-[#F0820B] hover:bg-orange-600 text-white text-sm sm:text-base font-semibold rounded-lg flex items-center justify-center gap-2 disabled:opacity-50 transition-all active:scale-95"
                    >
                      <img
                        src={LoginIcon}
                        alt="Login"
