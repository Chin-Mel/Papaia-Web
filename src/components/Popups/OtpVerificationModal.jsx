...
export default function OtpVerificationModal({ email, onSuccess }) {
  ...
  const handleVerify = async () => {
    ...
    if (enteredOtp.length !== 4) {
      showAlert("error", "Please enter the complete 4-digit OTP code.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data?.message?.toLowerCase().includes("expired")) {
          showAlert("error", "OTP has expired. Please request a new code.");
        } else if (
          data?.message?.toLowerCase().includes("invalid") ||
          data?.message?.toLowerCase().includes("incorrect")
        ) {
          showAlert("error", "Invalid OTP code. Please check and try again.");
        } else {
          showAlert(
            "error",
